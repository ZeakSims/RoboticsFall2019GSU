...
# a function that formats and save images captured
def capture(picNum):
    ...
    img.save("Module2_Problem{}.jpg".format(picNum))
...
def robotControl(trigger, simultaneous_launcher, motor_command_queue, sensor_queue):
    ...
    while not (trigger.is_set() or simultaneous_launcher.broken or gopigo3_robot.volt() <= MINIMUM_VOLTAGE):
        ...
        if move is False:
            ...
            # here we are displaying the distance again, but now if the distance is less then 500mm, it will capture a picture
            print("Distance Sensor Reading: {} mm ".format(dstnc))
            if dstnc<500 and picTaken is False:
                capture(picNum)
                picTaken=True
                picNum+=1
            if dstnc>500:
